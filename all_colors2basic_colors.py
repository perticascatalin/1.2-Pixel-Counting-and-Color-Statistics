# output statistics nicer
# first assign 0,1,2...
# and only when you have to output convert to rgb
...
def assignColor(pixel):
	...
	return best_color, id_best

def getBestColorMatch(image, saveFile):
	...
	for idx in range(8):
		fout.write(str(bins[idx]) + ' ' + str(float(bins[idx])/float(total)))
		fout.write('\n')
...
def colorAllImagesIn(path):
	...
	for image_name in files:
		...
		color_image = getBestColorMatch(image, image_name[0:len(image_name)-4] + '.stat')
		#cv2.imwrite('color_' + image_name, color_image)
